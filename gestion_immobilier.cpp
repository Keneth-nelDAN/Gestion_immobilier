...
public:
    ...
    proprietaire(string np, string pr, string nais, string adr, int tel, string titrepropri, string notarie, string nombanq, string numbanq, string numfis)
...
int main() {
    vector<proprietaire> proprietaires;
    vector<locataire> locataires;
    vector<bienImmobilier> biens;
    vector<contratLocation> contrats;

    ofstream fichier("donnees.txt", ios::app);
    if (fichier.tellp() == 0) {
        fichier << "===== PROPRIETAIRES =====\n";
        fichier << "Nom|Prenom|DateNaissance|Adresse|Telephone|TitrePropriete|ActeNotarie|Banque|NumBanquaire|NumFiscal\n";
        fichier << "===== LOCATAIRES =====\n";
        fichier << "Nom|Prenom|DateNaissance|Adresse|Telephone|BulletinSalaire|AttestationEmploi|AvisImposition|AncienProprietaire\n";
        fichier << "===== BIENS IMMOBILIERS =====\n";
        fichier << "Reference|Type|Adresse|Superficie|Prix\n";
        fichier << "===== CONTRATS DE LOCATION =====\n";
        fichier << "NumeroContrat|DateDebut|DateFin|MontantLoyer|Caution|Conditions\n";
    }
    int n;
        ...
        case 2:
            {
                locataire l;
                l.saisir();
                l.sauvegarder(fichier);
                locataires.push_back(l);
                cout << "\nDonnees Locataire enregistrees." << endl;
            }
            break;
        case 3:
            for (auto& p : proprietaires) {
                p.afficher();
            }
            break;
        case 4:
            for (auto& l : locataires) {
                l.afficher();
            }
            break;
        case 5:
